...
export function SkillEffectivenessChart({
  skills,
  isLoading,
}: SkillEffectivenessChartProps) {
  if (isLoading) {
    return (
    ...
    return (
      <div className="rounded-lg border border-gray-200 bg-white p-6">
        ...
        <p className="text-sm text-gray-500">
          No evaluation data available yet. Rate some traces to see skill
          effectiveness.
        </p>
      </div>
    )
  ...
  const maxCount = Math.max(...skills.map((s) => s.evaluationCount))
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div
                  className={`flex h-8 w-8 items-center justify-center rounded-full ${getRatingBgColor(skill.averageRating)}`}
                >
                  {getRatingTrend(skill.averageRating)}
                </div>
                <div>
                  <p className="font-medium text-gray-900">{skill.name}</p>
                  <p className="text-xs text-gray-500">
                    {skill.evaluationCount} evaluation
                    {skill.evaluationCount !== 1 ? "s" : ""}
                  </p>
                </div>
              </div>
              <div className="text-right">
                <p
                  className={`text-lg font-semibold ${getRatingColor(skill.averageRating)}`}
                >
            ...
            <div className="mt-2">
              <div className="h-1.5 w-full overflow-hidden rounded-full bg-gray-100">
                <div
                  ...
                  style={{
                    width: `${(skill.evaluationCount / maxCount) * 100}%`,
                  }}
                />
              </div>
            </div>
