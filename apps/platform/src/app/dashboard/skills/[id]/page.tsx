...
export default function SkillDetailPage() {
  ...
  const fetchSkill = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/skills/${skillId}?organizationId=${activeOrg.id}`,
        { credentials: "include" }
      )
  ...
  }, [skillId, activeOrg?.id])

  useEffect(() => {
    if (!isOrgPending && activeOrg?.id) {
      fetchSkill()
    }
  }, [fetchSkill, isOrgPending, activeOrg?.id])
  ...
  if (isOrgPending || isLoading) {
    return (
      <div>
        <PageHeader title="Skill Details" />
        <LoadingState />
      </div>
    )
  }
      ...
      <div>
        ...
        <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-6 text-center">
          <p className="text-sm text-yellow-700">
            Please select an organization to view this skill.
          </p>
        </div>
      </div>
    )
      ...
      <div className="space-y-8">
        ...
        <div className="mx-auto max-w-lg rounded-lg border border-red-200 bg-white p-6">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Irreversible actions that affect this skill.
          </p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-red-600">
                      This action cannot be undone. The skill will be removed
