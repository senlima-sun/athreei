...
describe("getServerSession", () => {
  ...
  it("properly forwards cookies to API", async () => {
    ...
    const call = mockFetch.mock.calls[0] as unknown as [string, RequestInit & { cache?: string }];
    const [url, options] = call;
    expect(url).toContain("/api/auth/get-session");
    expect((options?.headers as Record<string, string>)?.Cookie).toBe("session_token=abc123; csrf_token=xyz789");
