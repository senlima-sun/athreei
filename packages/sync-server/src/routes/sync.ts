...
sync.get('/state', async (c) => {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to get sync state';
    return c.json<ErrorResponse>({ error: message }, 500);
  }
});
...
sync.get('/settings', async (c) => {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to get sync settings';
    return c.json<ErrorResponse>({ error: message }, 500);
  }
});
