 ...
 */
traces.post("/", zValidator("json", TraceUploadRequestSchema), async (c) => {
  try {
    ...
    try {
      ...
      const result = await db
        ...
        .onConflictDoNothing({
          target: [schema.traces.account_id, schema.traces.request_id],
        })
    ...
    const message =
      error instanceof Error ? error.message : "Failed to upload traces"
 ...
 */
traces.get("/", zValidator("query", TraceQuerySchema), async (c) => {
  try {
    const { accountId } = getAuthContext(c)
 ...
 */
traces.delete("/", zValidator("json", TraceBulkDeleteSchema), async (c) => {
  try {
    const { accountId } = getAuthContext(c)
    ...
    if (conditions.length === 1) {
      return c.json<ErrorResponse>(
        {
          error:
            "At least one filter (traceIds, before, namespace, or endpoint) is required",
        },
        400
      )
    }
    ...
    const message =
      error instanceof Error ? error.message : "Failed to delete traces"
    return c.json<ErrorResponse>({ error: message }, 500)
  }
})
