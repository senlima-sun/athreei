...
account.get("/encryption-salt", async (c) => {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to get encryption salt"
...
account.post(
  ...
  async (c) => {
    try {
      ...
      const message =
        error instanceof Error ? error.message : "Failed to set encryption salt"
      return c.json<ErrorResponse>({ error: message }, 500)
    }
  }
)
...
account.delete("/encryption-salt", async (c) => {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Failed to clear encryption salt"
    return c.json<ErrorResponse>({ error: message }, 500)
  }
})
