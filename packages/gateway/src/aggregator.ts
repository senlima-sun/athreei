 ...
 */
export function createPrefixedName(serverName: string, toolName: string): string {
 ...
 */
export function aggregateTools(mcps: ConnectedMcp[]): AggregatedTool[] {
  const aggregated: AggregatedTool[] = [];
  const seenNames = new Set<string>();

  for (const mcp of mcps) {
    if (!mcp.tools || mcp.tools.length === 0) {
      log.debug(`Server "${mcp.config.name}" has no tools`);
      continue;
    }
    ...
    for (const tool of mcp.tools) {
      ...
      if (seenNames.has(prefixedName)) {
        log.warn(`Duplicate tool name detected: ${prefixedName}, skipping`);
        continue;
      }
