...
import type { EncryptedToolCallTrace, ToolCallTrace, NamespaceConfig } from "./types.js";
 ...
 */
function createEncryptedPayload(encrypted: EncryptedToolCallTrace["encryptedPayload"]): string {
  const payload = {
...
export class TraceSyncClient {
  ...
  isEncryptionEnabled(): boolean {
    return this.config.encryptionKey !== undefined && this.config.encryptionKey.length === 32;
  }
  ...
  private traceToUploadItem(trace: ToolCallTrace): TraceUploadItem {
    if (!this.isEncryptionEnabled()) {
      throw new Error("Encryption key not set - cannot sync traces without encryption");
    }
  ...
  async flush(): Promise<TraceUploadResponse> {
    ...
    try {
      const uploadItems = tracesToSend.map((trace) => this.traceToUploadItem(trace));
