...
describe("auth schema", () => {
  describe("user table", () => {
    ...
    it("should have optional image field", () => {
      const columns = getTableColumns(user)
      expect(getColumnConfig(columns.image as SQLiteColumn).notNull).toBe(false)
    })
  ...
  describe("session table", () => {
    ...
    it("should have token as unique", () => {
      const columns = getTableColumns(session)
      expect(getColumnConfig(columns.token as SQLiteColumn).isUnique).toBe(true)
    })

    it("should have userId as not null (foreign key)", () => {
      const columns = getTableColumns(session)
      expect(getColumnConfig(columns.userId as SQLiteColumn).notNull).toBe(true)
    })
  })
