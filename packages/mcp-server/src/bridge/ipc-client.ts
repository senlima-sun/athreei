...
export class IPCClient {
  ...
  private handleMessage(message: { id?: string; success?: boolean; payload?: unknown; error?: string }): void {
  ...
  async sendRequest<T = unknown>(method: string, payload: Record<string, unknown> = {}, timeout = 30000): Promise<T> {
  ...
  private async connectWithRetry(maxAttempts = 10, delay = 1000): Promise<void> {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (attempt === maxAttempts) {
          throw new Error(`Failed to connect after ${maxAttempts} attempts: ${err}`);
        }
